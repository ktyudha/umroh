...
@section('content')
    ...
    <div class="mt-10 dark:bg-gray-800 rounded-lg p-6">
        <form class="form-horizontal flex flex-col gap-4" id="form-itineraries"
            action="{{ route('admin.itineraries.store') }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            ...
            <div class="grid lg:grid-cols-3 grid-cols-1 gap-6">
                    ...
                    <select id="pilgrimage_batch_id" name="pilgrimage_batch_id" placeholder="Search a batch"
                        ...
                        @foreach ($pilgrimageBatches as $batch)
                            <option value="{{ $batch->id }}"
                                {{ old('pilgrimage_batch_id') == $batch->id ? 'selected' : '' }}>
                                {{ ucfirst($batch->name) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" id="location" name="location"
                        ...
                        placeholder="Location of itinerary" value="{{ old('location') }}" required />
                </div>
