...
@php
    $title = 'Pilgrimage Types';
    $breadcrumbs = [
        ['label' => 'Home', 'url' => route('admin.index')],
        ['label' => $title, 'url' => route('admin.pilgrimage-type.index')],
        ['label' => 'Edit', 'url' => null],
    ];
@endphp
...
@section('content')
    ...
    <div class="mt-10 dark:bg-gray-800 rounded-lg p-6">
        <form class="form-horizontal flex flex-col gap-4" id="form-posts"
            action="{{ route('admin.pilgrimage-type.update', $model->id) }}" method="post">
            {{ method_field('put') }}
            {{ csrf_field() }}
            <div>
                ...
                <input type="text" id="name" name="name"
                    ...
                    placeholder="Name of pilgrimage type" value="{{ old('name') ?: $model->name }}" required />
            </div>
            <div>
                <label class="mb-3 block text-sm font-medium text-black dark:text-white">
                    Description
                ...
                <input type="text" id="description" name="description"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="Description of pilgrimage type" value="{{ old('description') ?: $model->description }}" />
            </div>
            <div class="mr-auto">
                <button type="submit"
