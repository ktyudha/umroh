...
@section('content')
    ...
    <div class="dark:bg-gray-800 rounded-lg p-6">
        <form class="form-horizontal flex flex-col gap-4" id="form-transportations"
            action="{{ route('admin.transportations.update', $model->id) }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('put') }}
            <div>
            ...
            <div class="grid grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="text" id="code" name="code"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Code of transportations" value="{{ old('code') ?: $model->code }}" maxlength="3"
                        required />
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="capacity" name="capacity"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Capacity of transportations" value="{{ old('capacity') ?: $model->capacity }}"
                        required />
                </div>
                ...
                <div>
                    ...
                    <select id="class" name="class"
                        ...
                        @foreach ($trasnportationClasses as $key => $class)
                            <option value="{{ $class }}" {{ @$model->class == $class ? 'selected' : '' }}>
                                {{ ucfirst($class) }}
                            </option>
                        @endforeach
                    </select>
