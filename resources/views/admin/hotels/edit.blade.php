...
@section('content')
    ...
    <div class="dark:bg-gray-800 rounded-lg p-6">
        <form class="form-horizontal flex flex-col gap-4" id="form-hotels"
            action="{{ route('admin.hotels.update', $model->id) }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('put') }}
            <div>
            ...
            <div class="grid lg:grid-cols-2 grid-cols-1 mb-4 gap-4">
                <div>
                    ...
                    <textarea name="address" id="address"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Address of hotels">{{ old('address') ?: $model->address }}</textarea>
                </div>
                <div>
                    <label class="mb-3 block text-sm font-medium text-black dark:text-white">
                        Gmap
                    ...
                    <textarea name="gmap" id="gmap"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Gmaps of hotels">{{ old('gmap') ?: $model->gmap }}</textarea>
                </div>
                <div>
                    <label class="mb-3 block text-sm font-medium text-black dark:text-white">
                    ...
                    <input type="text" id="price" name="price"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Price of pilgrimage batch" value="{{ old('price') ?: $model->price }}" />
                </div>
                        ...
                        @foreach (range(1, 5) as $i)
                            <div class="flex items-center">
                                <input id="radio-{{ $i }}" type="radio" value="{{ $i }}"
                                    ...
                                    {{ old('rating', $model->rating) == $i ? 'checked' : '' }}>
                                <label for="radio-{{ $i }}"
                ...
                @foreach (@$model->images as $image)
                    <div class="hotel-images">
                        <input
                            ...
                            id="file_input" aria-describedby="file_input_help" id="file input" type="file"
                            name="image-{{ $image->id }}"
                            onchange="document.querySelector('.hotel-images img#photo-old-{{ $image->id }}').src = window.URL.createObjectURL(this.files[0])"
                            accept="image/*">
                        ...
                        <img src="{{ asset('storage/' . @$image->image ?? 'static/admin/images/default.png') }}"
                            class="rounded object-cover h-48 w-48" alt="photo" id="photo-old-{{ $image->id }}">
                        <button type="button" onclick="imageRemove(this)"
                            class="mt-3 text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">
