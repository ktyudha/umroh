...
class BreadcrumbController extends Controller
{
    ...
    public function edit()
    {
        ...
        $gallery    = @Setting::key(Setting::GALLERY)->first()->value;
        $product    = @Setting::key(Setting::PRODUCT)->first()->value;
        $contact    = @Setting::key(Setting::CONTACT)->first()->value;
        $doctor     = @Setting::key(Setting::DOCTOR)->first()->value;
       
        $setting    = (object) compact('profile','service','partner','blog','page','gallery','product','contact','doctor');
    ...
    public function update(Request $request)
    {
        ...
        if ($request->hasFile('gallery')) {
            ...
            if ($gallery && Storage::exists($gallery->value)) {
                Storage::delete(@$gallery->value);
            }
       
            $path = $request->file('gallery')->store('setting');
        ...
        if ($request->hasFile('product')) {
            $product = Setting::where('key','product')->first();
            if ($product && Storage::exists($product->value)) {
                Storage::delete(@$product->value);
            }
       
            $path = $request->file('product')->store('setting');
